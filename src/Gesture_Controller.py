...
class HandRecog:
    ...
    def get_gesture(self):
        ...
        # PINCH detection
        if self.finger in [Gest.LAST3,Gest.LAST4] and self.get_dist([8,4]) < 0.12:  # increased threshold
        ...
        if self.frame_count > 3:  # reduced from 4
...
class Controller:
    ...
    framecount = 0
    prev_hand = None
    pinch_threshold = 0.02  # reduced for responsiveness

    # Overlay
    @staticmethod
    def show_overlay(label, value):
        overlay_height = 50
        overlay_width = 400
        overlay = np.zeros((overlay_height, overlay_width,3), dtype=np.uint8)
        cv2.rectangle(overlay,(0,0),(overlay_width-1, overlay_height-1),(255,255,255),2)
        bar_width = int(value*(overlay_width-4))
        cv2.rectangle(overlay,(2,2),(bar_width,overlay_height-2),(0,255,0),-1)
        cv2.putText(overlay,f"{label}: {int(value*100)}%",(10,overlay_height//2 +5),
    ...
    def changesystembrightness():
        ...
        currentBrightnessLv += Controller.pinchlv/20.0  # increased sensitivity
        ...
        Controller.show_overlay("Brightness", currentBrightnessLv)
    ...
    def changesystemvolume():
        ...
        currentVolumeLv += Controller.pinchlv/20.0  # increased sensitivity
        currentVolumeLv = min(max(currentVolumeLv,0.0),1.0)
        volume.SetMasterVolumeLevelScalar(currentVolumeLv,None)
        Controller.show_overlay("Volume", currentVolumeLv)
